'''
Fuck
Fixed for userbot.. By @hackerpem giving me idea @The_Siddharth_Nigam.
'''
...
async def startgmute(event):
    ...
    if user_id == (await borg.get_me()).id:	
        ...
        await event.edit("Bitch was gmuted by userbot!Now Just Shut Up!!")
    ...
    if is_muted(userid, "gmute"):
        return await event.edit("Bitch is already gmuted by userbot😷")
    try:
        mute(userid, "gmute")
...
async def endgmute(event):
    ...
    if event.fwd_from:
        ...
        await event.edit("Bitch was Sucessfully ungmuted by userbot now speak again!😐")
        await asyncio.sleep(3)
        private = True
    reply = await event.get_reply_message()
    if event.pattern_match.group(1) is not None:
    ...
    if not is_muted(userid, "gmute"):
        return await event.edit("Bitch was not gmuted by userbot")
    try:
        unmute(userid, "gmute")
    except Exception as e:
        await event.edit("Error occured!\nError is " + str(e))
